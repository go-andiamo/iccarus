...
func init() {
	defaultDecoders = map[string]func(raw []byte, hdrs []TagHeader) (any, error){
		string(TagCurve):                      curveDecoder,
		string(TagDescription):                descDecoder,
		string(TagDictionary):                 dictDecoder,
		string(TagGamutBoundaryDescription):   gbdDecoder,
		string(TagModularAB):                  modularDecoder,
		string(TagModularBA):                  modularDecoder,
		string(TagMeasurement):                measurementDecoder,
		string(TagMultiFunctionTable1):        mft1Decoder,
		string(TagMultiFunctionTable2):        mft2Decoder,
		string(TagMultiLocalizedUnicode):      mlucDecoder,
		string(TagParametricCurve):            parametricCurveDecoder,
		string(TagProfileSequenceDescription): pseqDecoder,
		string(TagProfileSequenceIdentifier):  psidDecoder,
		string(TagS15Fixed16ArrayType):        sf32Decoder,
		string(TagSignatureType):              sigDecoder,
		string(TagText):                       textDecoder,
		string(TagView):                       viewDecoder,
		string(TagXYZ):                        xyzDecoder,
		string("MSBN"):                        msbnDecoder,
		string(TagZXML):                       zxmlDecoder,
	}
}
